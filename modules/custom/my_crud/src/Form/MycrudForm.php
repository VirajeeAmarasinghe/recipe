...
class MycrudForm extends FormBase{
    ...
    public function validateForm(array &$form, FormStateInterface $form_state){
        ...
        if(!preg_match('/[a-zA-Z\s]/', $name)){
            $form_state->setErrorByName('name', $this->t('Name must be in Letters Only'));
        }

        $age = $form_state->getValue('age'); 

        if(!preg_match('/^[1-9][0-9]{0,3}$/', $age)){
    ...
    public function submitForm(array &$form, FormStateInterface $form_state){
        ...
        if(isset($_GET['id'])){
            ...
            \Drupal::messenger()->addMessage('Successfully Updated Records');
        }else{
            $field = ['name'=>$name, 'age'=>$age];
            $query = \Drupal::database();
            $query->insert('my_crud')->fields($field)->execute();
            \Drupal::messenger()->addMessage('Successfully Added New Records');
        }
